                ...
                <div class="update-available-card-wrapper">
                    @if (!extension_loaded('zip') && !gs('system_customized'))
                        <div class="card bl--5 border--warning mb-4">
                            <div class="card-body">
                                <p class="text--warning"><i class="las la-exclamation-triangle"></i> PHP-zip extension is required to perform the update operation.</p>
                            </div>
                        </div>
                    @endif
                    ...
                    @if (gs('system_customized'))
                        <div class="card bl--5 border--warning mb-4">
                            <div class="card-body">
                                <p class="text--warning"><i class="las la-exclamation-triangle"></i> The system already customized. You can't update the project.</p>
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="update-available-card {{ version_compare(gs('available_version'), systemDetails()['version'], '>') ? 'border--danger' : 'border--success' }}">
                        <div class="update-available-card-top">
                            ...
                            @if (version_compare(gs('available_version'), systemDetails()['version'], '>'))
                                <div class="update-available-card__item">
                                    ...
                                    <p class="text--success">Latest Version</p>
                                </div>
                            @endif
                        </div>
                        <div class="update-available-card-bottom">
                            ...
                            @else
                                @if (!gs('system_customized'))
                                    <p><span><i class="las la-info-circle text--primary"></i></span> <span>A new system version has already been released that you have not grabbed yet. Don't miss it out. Get the latest features of the system.</span></p>
                                @endif
                            @endif
                        </div>
                    </div>
                    @if(version_compare(gs('available_version'), systemDetails()['version'], '>'))
                    <button class="btn btn--primary w-100 h-45 mt-4" data-bs-toggle="modal" data-bs-target="#updateModal">Atualizar Now</button>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Atualizar the Sistema</h5>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn--dark disableBtn" data-bs-dismiss="modal">Fechar</button>
...
@push('breadcrumb-plugins')
    <a href="{{ route('admin.system.update.log') }}" class="btn btn-outline--primary btn-sm"><i class="las la-list"></i> Atualizar Log</a>
@endpush
