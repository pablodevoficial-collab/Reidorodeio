@php
    ...
    $photoUrl = $competitor?->foto_url ?: asset('assets/images/logo_icon/favicon.png');
    $ctaUrl = $event->cta_url ?: route('hub.stats', ['competitor' => $competitor?->id]);
    $ctaLabel = $event->cta_label ?: 'Acompanhar competidor';
@endphp
<!DOCTYPE html>
<html lang="pt-BR">
<head>
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:640px;background:linear-gradient(180deg,#10182c 0%,#0a1020 100%);border:1px solid rgba(250,174,33,.35);border-radius:26px;overflow:hidden;">
                    <tr>
                        <td style="padding:22px 24px;background:linear-gradient(135deg,#f7a600 0%,#f97316 100%);color:#1d1302;font-size:12px;font-weight:700;letter-spacing:.18em;text-transform:uppercase;">
                            Rei do Rodeio • Alerta de competidor
                        </td>
                    </tr>
                    <tr>
                        <td style="padding:28px 24px 18px;">
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                                <tr>
                                    <td width="92" valign="top" style="padding-right:18px;">
                                        <img src="{{ $photoUrl }}" alt="{{ $competitor?->nome }}" width="84" height="84" style="width:84px;height:84px;border-radius:22px;display:block;object-fit:cover;border:1px solid rgba(250,174,33,.45);background:#08101d;">
                                    </td>
                                    <td valign="top">
                                        <div style="font-size:13px;line-height:1.5;color:#f7b955;font-weight:700;letter-spacing:.08em;text-transform:uppercase;">Seu competidor teve movimentação</div>
                                        <h1 style="margin:8px 0 10px;font-size:28px;line-height:1.15;color:#ffffff;">{{ $event->title }}</h1>
                                        <p style="margin:0;font-size:15px;line-height:1.7;color:#c4d3ee;">{{ $event->message }}</p>
                                    </td>
                                </tr>
                            </table>
                        </td>
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#0d1629;border:1px solid rgba(129,153,194,.22);border-radius:20px;">
                                <tr>
                                    <td style="padding:18px 20px;">
                                        ...
                                        <div style="font-size:15px;color:#ffffff;font-weight:700;margin-bottom:8px;">{{ $competitor?->nome ?? 'Competidor' }}</div>
                                        @if($rodeio)
                                            <div style="font-size:14px;color:#c7d5ee;margin-bottom:6px;">Rodeio: {{ $rodeio->name }}</div>
                                        @endif
                                        @if($modalidade)
                                            <div style="font-size:14px;color:#c7d5ee;margin-bottom:6px;">Modalidade: {{ $modalidade->nome }}</div>
                                        @endif
                                        <div style="font-size:14px;color:#8cf2ae;font-weight:700;">{{ $event->created_at?->format('d/m/Y H:i') }}</div>
                                    </td>
                                </tr>
                            </table>
                        </td>
