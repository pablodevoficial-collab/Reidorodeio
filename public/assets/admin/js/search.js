...
const getSearchListItems = () => Array.from(document.querySelectorAll('.search-list li'));
const getSearchListLinks = () => Array.from(document.querySelectorAll('.search-list li a'));

if (searchInput) {
  searchInput.addEventListener('keydown', function (e) {
    const items = getSearchListItems();
    const length = items.length;
    if (search !== searchInput.value && process) {
      len = 0;
      clickLink = 0;
      if (items[len]) {
        const link = items[len].querySelector('a');
        if (link) link.focus();
      }
      searchInput.focus();
    }
    ...
    if (e.keyCode === 40 && length) {
      ...
      getSearchListLinks().forEach(a => a.classList.remove('text-active'));
      if (items[len]) {
        const link = items[len].querySelector('a');
        if (link) {
          link.focus();
      ...
      searchInput.focus();
      clickLink = len;
      if (!items[clickLink] || !items[clickLink].querySelector('a')) {
        if (items[len]) items[len].classList.add('text-active');
      }
      len += 1;
    ...
    //Enter
      ...
      e.preventDefault();
      if (items[clickLink]) {
        const link = items[clickLink].querySelector('a');
        if (link && process) {
          link.click();
        }
      }
