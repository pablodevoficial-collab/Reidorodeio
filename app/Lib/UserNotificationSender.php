...
class UserNotificationSender {
    ...
    private function manageSessionForNotification($totalUserCount, $request) {
        ...
        if ($sessionData['total_sent'] >= $totalUserCount) {
            ...
            // Rota de histórico de notificações no admin (ReportController)
            $url     = route("admin.report.notification.history");
        } else {
            ...
            $url     = route("admin.report.notification.history") . "?email_sent=yes";
        }
