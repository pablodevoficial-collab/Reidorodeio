...
class SubscriberController extends Controller {
    public function index() {
        ...
        if (!Schema::hasTable((new Subscriber())->getTable())) {
            return view('admin.feature_unavailable', [
                'pageTitle' => $pageTitle,
                'message' => 'Tabela de subscribers não está disponível neste banco. Importe as migrations/SQL para habilitar este módulo.',
            ]);
        }
    ...
    public function remove($id) {
        if (!Schema::hasTable((new Subscriber())->getTable())) {
            $notify[] = ['error', 'Subscribers module is not available (missing table).'];
